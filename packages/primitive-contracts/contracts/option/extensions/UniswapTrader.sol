...
contract UniswapTrader is Ownable {
    ...
    function mintAndMarketSell(
        ...
        (, bool success) = marketSell(msg.sender, msg.sender, address(option), outputOptions, minQuote);
    ...
    function marketSell(
        ...
        path[0] = option;
        path[1] = quoteToken;
        IERC20(option).approve(router, uint(-1));
        (amounts) = IUniswapV2Router02(router).swapExactTokensForTokens(sellQuantity, minQuote, path, to, getMaxDeadline());
