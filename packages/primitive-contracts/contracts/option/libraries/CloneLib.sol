...
library CloneLib {
     ...
     * @param target The address of the template contract to derive clone bytecode for.
     */
    function computeCreationCode(address deployer, address target) internal pure returns (bytes memory clone) {
        ...
        assembly {
            ...
            mstore(add(clone, 0x2d), mul(deployer, 0x01000000000000000000000000))
        ...
        for (uint i = 0; i < consData.length; i++) {
