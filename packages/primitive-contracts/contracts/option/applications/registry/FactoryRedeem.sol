...
} from "./RedeemImplementationLauncherLib.sol";
...
contract FactoryRedeem is Ownable, NullCloneConstructor {
    ...
    function deployRedeemImplementation() public {
        redeemImplementationAddress = RedeemImplementationLauncherLib
        ...
        returns (address redeem)
    {
        bytes32 salt = keccak256(
            abi.encodePacked(
                RedeemImplementationLauncherLib.REDEEM_SALT(),
                owner(),
                tokenP,
                underlying
            )
        );
        redeem = FactoryLib.create2Clone(
            redeemImplementationAddress,
            uint256(salt)
        );
