...
const wrapLog = (log, contract) =>
    Object.assign(log, {
        getBlock: async () => await contract.provider.getBlock(log.blockHash),
        getTransaction: async () =>
            await contract.provider.getTransaction(log.transactionHash),
        getTransactionReceipt: async () =>
            await contract.provider.getTransactionReceipt(log.transactionHash),
    });
...
class EthersBaseClass {
    ...
    async getEvents(filterOpts = {}) {
        ...
        const address =
            this.contract.address.length === 66
                ? "0x" + this.contract.address.substr(26)
        ...
        const logs = (await this.contract.provider.getLogs(filter)).map((v) =>
            wrapLog(
                Object.assign(v, {
...
const makeEthersBaseClass = (artifact) => {
    ...
    const contract = new ethers.Contract(
        ZERO_ADDRESS,
        abi,
        new ethers.providers.InfuraProvider("kovan")
    );
    ...
    const baseClass = class PrimitiveEthersBaseClass extends EthersBaseClass {
        static get(network) {
            ...
            return new this(
                ((this.networks || {})[chainId] || {}).address ||
                    ethers.constants.AddressZero,
                tryToGetInfura(chainId)
            );
        }
        ...
        constructor(address, providerOrSigner) {
            super();
            address =
                address.length === 66 ? "0x" + address.substr(26) : address;
            this.contract = contract
                .attach(address)
                .connect(toSigner(providerOrSigner));
            ...
            try {
                ...
                return ethers.utils.hexlify(
                    ethers.utils.parseUnits(
                        String(Math.floor(Number(result) * Number(multiplier))),
                        8
                    )
                );
            } catch (e) {
        ...
        async addGasPrice(args) {
            ...
            if (gasPrice) {
                const last =
                    ...
                    Object.getPrototypeOf(args[args.length - 1]) ===
                        Object.prototype
                        ? args[args.length - 1]
                        : (() => {
                              const last = {};
    ...
    Object.keys(contract.functions).forEach((v) => {
        managerSubclass.prototype[v] = async function (...args) {
            ...
            const isCall =
                keyed[v].constant ||
                ["view", "pure"].includes(keyed[v].stateMutability);
            const mappedArgs = isCall ? args : await this.addGasPrice(args);
            const innerContract = isCall
                ? contract
                      .attach(this.address)
                      .connect(
                          this.contract.provider.asEthers
                              ? this.contract.provider.asEthers()
                              : this.provider
                      )
                : contract
                      .attach(this.address)
                      .connect(toSigner(this.contract.provider));
            ...
            const listener = async (block) => {
                while (true) {
                    block = await provider.send("eth_getBlockByNumber", [
                ...
                const actualTx = block.transactions.find(
                    (v) => v.from === tx.from && nonce === tx.nonce
                );
                if (actualTx) {
                    ...
                    receipt.logs = receipt.logs.map((v) => {
                        try {
                            Object.assign(
                                v,
                                this.constructor.interface.parseLog(v)
                            );
                        } catch (e) {}
                        return v;
                    });
...
const copyStaticsFromArtifact = (target, artifact) => {
    const interfaceObject = new ethers.utils.Interface(
        uniqBy(artifact.abi, "name")
    );
...
const makeMainnetInstanceGetterFactory = (artifact) => {
    ...
    baseClass.getMainnetInstance = function (provider) {
        ...
        if (!artifact.networks[1])
            throw Error(
                "no mainnet address associated with factory for " +
