...
describe("Option Contract", () => {
    ...
    describe("Option", () => {
        ...
        it("should return the correct strikeToken", async () => {
            assert.equal(
                (await optionToken.getStrikeTokenAddress()).toString(),
                strikeToken.address,
        ...
        it("should return the correct base", async () => {
            assert.equal(
                (await optionToken.getBaseValue()).toString(),
                base,
                "Incorrect base"
            );
        });
        ...
        it("should return the correct quote", async () => {
            assert.equal(
                (await optionToken.getQuoteValue()).toString(),
                quote,
                "Incorrect quote"
            );
        });
        ...
        it("should return the correct expiry", async () => {
            assert.equal(
                (await optionToken.getExpiryTime()).toString(),
                expiry,
                "Incorrect expiry"
            );
        });
        ...
        describe("mintOptions()", () => {
            ...
            mint = async (inputUnderlyings) => {
                ...
                let optionsChange = (
                    await getTokenBalance(optionToken, Alice)
                ).sub(optionBalance);
                let redeemsChange = (
                    await getTokenBalance(redeemToken, Alice)
                ).sub(redeemBalance);
            ...
            it("revert if no tokens were sent to contract", async () => {
                await expect(optionToken.mintOptions(Alice)).to.be.revertedWith(
                    ERR_ZERO
                );
            });
        ...
        describe("exerciseOptions()", () => {
            ...
            exercise = async (inputOptions) => {
                ...
                await strikeToken.transfer(
                    optionToken.address,
                    inputStrikes.add(fee)
                );

                await expect(
                    optionToken.exerciseOptions(Alice, inputOptions, [])
                )
                ...
                let strikesChange = (
                    await getTokenBalance(strikeToken, Alice)
                ).sub(strikeBalance);
            ...
            it("revert if 0 underlyingToken requested to be taken out", async () => {
                await expect(
                    optionToken.exerciseOptions(Alice, 0, [])
        ...
        describe("closeOptions()", () => {
            ...
            it("revert if 0 redeemToken were sent to contract", async () => {
                ...
                await optionToken.transfer(optionToken.address, inputOptions);
                await expect(
                    optionToken.closeOptions(Alice)
                ).to.be.revertedWith(ERR_ZERO);
            });
        ...
        describe("update", () => {
            ...
            it("should update the cached balances with the current balances", async () => {
                ...
                await expect(optionToken.updateCacheBalances()).to.emit(
                    optionToken,
                ...
                let underlyingBalance = await getTokenBalance(
                    underlyingToken,
                    optionToken.address
                );
                let strikeBalance = await getTokenBalance(
                    strikeToken,
                    optionToken.address
                );
        ...
        describe("test expired", () => {
            ...
            it("revert when calling swap on an expired optionToken", async () => {
                await expect(
                    optionToken.exerciseOptions(Alice, 1, [])
