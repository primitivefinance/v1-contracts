...
describe("Trader", () => {
    ...
    describe("safeMint", () => {
        safeMint = async (inputUnderlyings) => {
            ...
            await expect(
                trader.safeMint(optionToken.address, inputUnderlyings, Alice)
            )
    ...
    describe("safeRedeem", () => {
        ...
        safeRedeem = async (inputRedeems) => {
            ...
            await expect(
                trader.safeRedeem(optionToken.address, inputRedeems, Alice)
            )
    ...
    describe("safeClose", () => {
        ...
        safeClose = async (inputOptions) => {
            ...
            await expect(
                trader.safeClose(optionToken.address, inputOptions, Alice)
            )
    ...
    describe("safeUnwind", () => {
        ...
        safeUnwind = async (inputOptions) => {
            ...
            await expect(
                trader.safeUnwind(optionToken.address, inputOptions, Alice)
            )
    ...
    describe("test bad ERC20", () => {
        ...
        it("should revert on mint because transfer does not return a boolean", async () => {
            ...
            await expect(
                trader.safeMint(optionToken.address, HUNDRED_ETHER, Alice)
            ).to.be.reverted;
        });

        it("should revert on swap because transfer does not return a boolean", async () => {
            await expect(
                trader.safeExercise(optionToken.address, HUNDRED_ETHER, Alice)
            ).to.be.reverted;
        });
        ...
        it("should revert on redeemToken because transfer does not return a boolean", async () => {
            ...
            await expect(
                trader.safeRedeem(optionToken.address, inputStrikes, Alice)
            ).to.be.reverted;
        });

        it("should revert on close because transfer does not return a boolean", async () => {
            await expect(
                trader.safeClose(optionToken.address, HUNDRED_ETHER, Alice)
            ).to.be.reverted;
        });

        it("should revert on unwind because its not expired yet", async () => {
            await expect(
                trader.safeUnwind(optionToken.address, HUNDRED_ETHER, Alice)
            ).to.be.reverted;
        });
    });
});
