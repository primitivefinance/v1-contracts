...
const getTokenAddressArrays = async (signer) => {
    ...
    let redeemAddressArray = [];
    let underlyingAddressArray = [];
    for (let x = 0; x < optionAddressArray.length; x++) {
        let option = optionAddressArray[x];
        let optionToken = new ethers.Contract(option, Option.abi, signer);
        let redeem = await optionToken.redeemToken();
        let underlying = await optionToken.getUnderlyingTokenAddress();
        redeemAddressArray.push(redeem);
        underlyingAddressArray.push(underlying);
    }
    return { optionAddressArray, redeemAddressArray, underlyingAddressArray };
};

const writeUniswapJson = (optionJsonObject) => {
    let data = JSON.stringify(optionJsonObject, null, 2);
    fs.writeFileSync("./uniswap_pairs_short.json", data);
};
 ...
 */
const addUniswapV2Liquidity = async (
    optionAddress,
    redeemAddress,
    tokenAddress
) => {
    ...
    const uniswapTrader = await getInstance("UniswapConnector02", signer);
    ...
    // Create a new pair if it does not exist (equal to address zero).
    let pairAddress = await getUniswapV2Pair(redeemAddress, tokenAddress);
    ...
    let base = await optionTokenInstance.getBaseValue();
    let quote = await optionTokenInstance.getQuoteValue();

    // approve the router to take the option liquidity
    let redeemTokenInstance = new ethers.Contract(
        redeemAddress,
        USDC.abi,
        signer
    );

    let otherTokenInstance = new ethers.Contract(
        tokenAddress,
        USDC.abi,
        signer
    );

    await checkAllowance(signer, uniswapRouter, redeemTokenInstance);
    await checkAllowance(signer, uniswapRouter, otherTokenInstance);
    ...
    let redeemProvision = parseEther("5000");
    let underlyingProvision = redeemProvision.mul(base).div(quote);
    // Add liquidity to Uniswap pool.
    try {
        await uniswapRouter.addLiquidity(
            redeemAddress, // token 0
            tokenAddress, // token 1
            redeemProvision, // quantity of token 0
            underlyingProvision, // quantity of token 1
            0, // min quantity of lp tokens
            0, // min quantity of lp tokens
            account, // lp token receiver
            deadline // deadline until trade expires
        );
    } catch (err) {
...
async function main() {
    ...
    const {
        optionAddressArray,
        redeemAddressArray,
        underlyingAddressArray,
    } = await getTokenAddressArrays(signer);
    ...
    console.log(redeemAddressArray.length);
    for (let i = 0; i < redeemAddressArray.length; i++) {
        let optionAddress = optionAddressArray[i];
        let redeemAddress = redeemAddressArray[i];
        let underlyingAddress = underlyingAddressArray[i];
